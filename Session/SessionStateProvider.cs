...
namespace MemcachedProviders.Session
{
	public class SessionStateProvider:SessionStateStoreProviderBase
	{
		...
		public override void Initialize(string name, System.Collections.Specialized.NameValueCollection config)
		{
			...
			if(!string.IsNullOrEmpty(config["dbType"]))
			{
				...
				else
				{
					...
					if(_objConnectionStringSettings == null || _objConnectionStringSettings.ConnectionString.Trim() == "")
					{
		...
		public override void CreateUninitializedItem(System.Web.HttpContext context, string id, int timeout)
		{
			...
			if(this._bIsDbNone == false)
			{
				...
				using(IDbOperations objDb = DbFactory.CreateDbOperations(_strDbType, _strConn))
				{
					objDb.Add(
						id, ApplicationName,
						dSetTime, dSetTime.AddMinutes((double)timeout), dSetTime,
						0, timeout,
						false,
						null,
						1
			...
			#region Обновление объекта в сессии
		...
		/// <summary>
		/// Возвращает сессию для режима "только для чтения" без необходимости блокировки
		/// </summary>
		public override SessionStateStoreData GetItem(System.Web.HttpContext context, string id, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actions)
		{
			...
			if(objHolder != null)
			{
				if(objHolder.Locked == false)
				{
					...
					this._client.Store(StoreMode.Set, id, objHolder, new TimeSpan(0, 0, 0, 0, _objConfig.Timeout.Milliseconds));

					actions = (SessionStateActions)objHolder.ActionFlag;
					lockId = objHolder.LockId;
					lockAge = objHolder.LockAge;

					if(this._bIsDbNone == false) // Сохранение в БД
					{
						using(IDbOperations objDb = DbFactory.CreateDbOperations(_strDbType, _strConn))
						{
			...
			else
			{
				if(this._bIsDbNone == false)
				{
					using(IDbOperations objDb = DbFactory.CreateDbOperations(_strDbType, _strConn))
					{
						return objDb.GetItem(id, ApplicationName, _objConfig.Timeout.Minutes, context, false, out locked, out lockAge, out lockId, out actions);
					}
				}
				else
				{
		...
		/// <summary>
		/// Возвращает сессию в режиме эксклюзивного доступа с необходимостью блокировки
		/// </summary>
		public override SessionStateStoreData GetItemExclusive(System.Web.HttpContext context, string id, out bool locked, out TimeSpan lockAge, out object lockId, out SessionStateActions actions)
		{
			...
			if(objHolder != null)
			{
				if(objHolder.Locked == false)
				{
					...
					// Блокирование элемента в memcached
			...
			else
			{
				if(this._bIsDbNone == false) // Сохранение в БД
				{
					using(IDbOperations objDb = DbFactory.CreateDbOperations(_strDbType, _strConn))
					{
						return objDb.GetItem(id, ApplicationName, _objConfig.Timeout.Minutes, context, true, out locked, out lockAge, out lockId, out actions);
					}
				}
				else
				{
		...
		/// <summary>
		/// Снимает блокировку сессии, освобождая ее для других потоков
		/// </summary>
		public override void ReleaseItemExclusive(System.Web.HttpContext context, string id, object lockId)
		{
			#region Обновление элемента в memcached
			MemcachedHolder objHolder = this._client.Get<MemcachedHolder>(id);
		...
		{
			this._client.Remove(id); // Удаление элемента из memcached

			if(this._bIsDbNone == false)
			{
				#region Удаление элемента из БД
				using(IDbOperations objDb = DbFactory.CreateDbOperations(_strDbType, _strConn))
				{
		...
		public override void ResetItemTimeout(System.Web.HttpContext context, string id)
		{
			...
			if(this._bIsDbNone == false)
			{
				#region Обнуляет TimeOut у элемента в БД
				using(IDbOperations objDb = DbFactory.CreateDbOperations(_strDbType, _strConn))
				{
		...
		/// <summary>
		/// Сохраняет состояние сессии и снимает блокировку
		/// </summary>
		public override void SetAndReleaseItemExclusive(System.Web.HttpContext context, string id, SessionStateStoreData item, object lockId, bool newItem)
		{
			if(this._bIsDbNone == false)
			{
				...
				using(IDbOperations objDb = DbFactory.CreateDbOperations(_strDbType, _strConn))
				{
					...
					{
						// Сохранение в БД
						objDb.Add(id, ApplicationName, dSetTime, dSetTime.AddMinutes((Double)item.Timeout), dSetTime, 0, item.Timeout, false, objContent, 0);

						// Сохранение в memcached
						var objHolder = new MemcachedHolder(objContent, false, dSetTime, 0, 0);
						this._client.Store(StoreMode.Set, id, objHolder, new TimeSpan(0, item.Timeout, 0));
					}
					else
					{
						// Сохранение в БД
						objDb.Update(id, ApplicationName, (int)lockId, dSetTime.AddMinutes((Double)item.Timeout), objContent, false);
